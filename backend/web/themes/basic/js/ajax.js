(function ($, window, Yiipal, yiipalSettings) {
     ...
     *
     * Yiipal uses this file to enhance form elements with #ajax['url'] and
    ...
    Yiipal.behaviors.AJAX = {
        attach: function (context, settings) {
            function loadAjaxBehavior(base) {
                ...
                $(element_settings.selector).once('yiipal-ajax').each(function () {
                    element_settings.element = this;
                    Yiipal.ajax[element_settings.selector] = new Yiipal.ajax(base, this, element_settings);
                });
            }
            ...
            $('.use-ajax').once('ajax').each(function () {
                ...
                Yiipal.ajax[baseUseAjax] = new Yiipal.ajax(baseUseAjax, this, element_settings);
            });
            ...
            $('.use-ajax-submit').once('ajax').each(function () {
                ...
                Yiipal.ajax[baseUseAjaxSubmit] = new Yiipal.ajax(baseUseAjaxSubmit, this, element_settings);
            });
        }
    };
     ...
     */
    Yiipal.AjaxError = function (xmlhttp, uri) {
        ...
        var readyStateText;
        if (xmlhttp.status) {
            statusCode = "\n" + Yiipal.t("An AJAX HTTP error occurred.") + "\n" + Yiipal.t("HTTP Result Code: !status", {'!status': xmlhttp.status});
        }
        else {
            statusCode = "\n" + Yiipal.t("An AJAX HTTP request terminated abnormally.");
        }
        statusCode += "\n" + Yiipal.t("Debugging information follows.");
        pathText = "\n" + Yiipal.t("Path: !uri", {'!uri': uri});
        statusText = '';
        // In some cases, when statusCode === 0, xmlhttp.statusText may not be defined.
        // Unfortunately, testing for it with typeof, etc, doesn't seem to catch that
        // and the test causes an exception. So we need to catch the exception here.
        try {
            statusText = "\n" + Yiipal.t("StatusText: !statusText", {'!statusText': $.trim(xmlhttp.statusText)});
        }
        catch (e) {
            // empty
        }
        ...
        try {
            responseText = "\n" + "ResponseText: !responseText", {'!responseText': $.trim(xmlhttp.responseText)};
        }
        catch (e) {
            // empty
        }
        ...
        readyStateText = xmlhttp.status === 0 ? ("\n" + "ReadyState: !readyState", {'!readyState': xmlhttp.readyState}) : "";

        this.message = statusCode + pathText + statusText + responseText + readyStateText;
        this.name = 'AjaxError';
    };

    Yiipal.AjaxError.prototype = new Error();
    Yiipal.AjaxError.prototype.constructor = Yiipal.AjaxError;
        ...
        changed: function (ajax, response, status) {
            if (!$(response.selector).hasClass('ajax-changed')) {
                ...
                if (response.asterisk) {
                    $(response.selector).find(response.asterisk).append(' <abbr class="ajax-changed" title="' + 'Changed' + '">*</abbr> ');
                }
            }
        },
...
})(jQuery, this, Yiipal, yiipalSettings);